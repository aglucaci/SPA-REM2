...
def GeneratePlot(species, YAxis, YAxisTitle):
    ...
    r_squared_annotation = regression_stats.mark_text(
        ...
        x=alt.value(200),
        y=alt.value(20),
        text = alt.Text(['slope:N', 'intercept:N'], title="Slope"),
        color=alt.Color(legend=None)
    )

    # Combine charts
    outputFilenamePNG = 'FIXED-SPA-REM2-' + YAxis.replace(" ", "") + '-TN93-' + species + '.png'
    outputFilenameSVG = 'FIXED-SPA-REM2-' + YAxis.replace(" ", "") + '-TN93-' + species + '.svg'
